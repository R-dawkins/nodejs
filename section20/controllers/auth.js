...
const { GMAIL_PASS, GMAIL_ID } = process.env;
...
exports.postSignup = (req, res, next) => {
  ...
  // user가 이미 존재할 시 res.redirect로 페이지를 이동하지만 다음의 promise.then은 그대로 실행되기 때문에 hash에 바로 then을 붙인다.
  return bcrypt
    .hash(password, 12)
    .then((hashedPassword) => {
      const user = new User({
    ...
    .then((result) => {
      res.redirect("/login");
      return transporter.sendMail(signupMail);
    })
    .catch((err) => {
      {
        const error = new Error(err);
